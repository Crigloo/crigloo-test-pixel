___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.

___INFO___

{
  "displayName": "AdMaxim Retargeting Pixel",
  "description": "AdMaxim re-targeting pixel helps you to re-target those user's who visit's your website.",
  "categories": ["REMARKETING","LEAD_GENERATION","ADVERTISING"],
  "securityGroups": [],
  "id": "cvt_temp_public_id",
  "type": "TAG",
  "version": 1,
  "brand": {
    "thumbnail": "data:image/png;base64,",
    "displayName": "",
    "id": "brand_dummy"
  },
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "help": "Please copy & paste Pixel ID from AdMaxim Campaign Manager.",
    "valueValidators": [
      {
        "type": "NON_EMPTY"
      },
      {
        "args": [
          "^[0-9]+$"
        ],
        "type": "REGEX"
      }
    ],
    "displayName": "Pixel ID",
    "simpleValueType": true,
    "name": "pid",
    "type": "TEXT"
  },
  {
    "help": "Please copy & paste Segment ID from AdMaxim Campaign Manager.",
    "valueValidators": [
      {
        "type": "NON_EMPTY"
      },
      {
        "args": [
          "^[0-9]+$"
        ],
        "errorMessage": "",
        "type": "REGEX"
      }
    ],
    "displayName": "Segment ID",
    "simpleValueType": true,
    "name": "segmentId",
    "type": "TEXT"
  }
]


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "logging",
        "versionId": "1"
      },
      "param": [
        {
          "key": "environments",
          "value": {
            "type": 1,
            "string": "debug"
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "send_pixel",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urls",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 1,
                "string": "https://datatrack.admaxim.com/*"
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "get_url",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urlParts",
          "value": {
            "type": 1,
            "string": "any"
          }
        },
        {
          "key": "queriesAllowed",
          "value": {
            "type": 1,
            "string": "any"
          }
        }
      ]
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "get_referrer",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urlParts",
          "value": {
            "type": 1,
            "string": "any"
          }
        },
        {
          "key": "queriesAllowed",
          "value": {
            "type": 1,
            "string": "any"
          }
        }
      ]
    },
    "isRequired": true
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

//API import
const sendPixel = require('sendPixel');
const getTimestamp = require('getTimestamp');
const getReferrerUrl = require('getReferrerUrl');
const getUrl = require('getUrl');
const encodeUriComponent = require('encodeUriComponent');
const makeString = require('makeString');

//variables
const pid = data.pid;
const segmentId = data.segmentId;
var log = require('logToConsole');

const trackingUrl = 'https://datatrack.admaxim.com/datatracker/rdata?px='+encodeUriComponent(makeString(pid))+'&segid='+encodeUriComponent(makeString(segmentId))+'&ts='+encodeUriComponent(makeString(getTimestamp()))+'&ref='+encodeUriComponent(getReferrerUrl())+'&url='+encodeUriComponent(getUrl());

sendPixel(trackingUrl, data.gtmOnSuccess, data.gtmOnFailure, 'retargeting pixel');

log('px='+encodeUriComponent(makeString(pid))+',segid='+encodeUriComponent(makeString(segmentId))+',ts='+encodeUriComponent(makeString(getTimestamp()))+',ref='+encodeUriComponent(getReferrerUrl())+',url='+encodeUriComponent(getUrl()));
log('data =', data);


___NOTES___

Created on 18/09/2019, 12:05:34
